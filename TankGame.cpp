...
//    SetConsoleScreenBufferSize(hStdOut, sizePos);
    ...
    // 分配游戏空间状态表
    gameSpace = new GameSpaceUnit *[high];
    for (int i = 0; i != high; i++)
        gameSpace[i] = new GameSpaceUnit[wide];
...
void TankGame::enter_picture(char &command) {
    ...
    for (int i = 0; i != 6; i++)
        printWin(7, 6 + i,BLACK, captions[i]);

    Frame frame(30, 15);
    frame.draw_frame(4, 19);
    ...
    for (int i = 0; i != 3; i++) {
        for (int k = 0; k != sentence[i].size(); k++) {
            printWin(14 + k, 21 + 2 * i, BLACK,sentence[i][k]);
            sleep(50);
        }
        sleep(1000);
    }

    printWin(7, 28, BLACK,"Enter : 开始              ESC : 退出");
...
void TankGame::turn_to_next_task(char &command) {
    clear_picture();
    printWin(18, 20,BLACK, "第");
    printWin(20, 20, BLACK, no);
    printWin(22, 20, BLACK, "关");
...
void TankGame::success_gameover() {
    ...
    for (int i = 0; i != 3; i++) {
        for (int k = 0; k != sentence[i].size(); k++) {
            printWin(12 + k, 14 + 2 * i, BLACK,sentence[i][k]);
            sleep(50);
        }
        sleep(1000);
    }
...
void TankGame::game_over() {
    ...
    for (int i = 0; i != 4; i++) {
        for (int k = 0; k != sentence[i].size(); k++) {
            printWin(13 + k, 18 + 2 * i,BLACK, sentence[i][k]);
            sleep(50);
        }
        sleep(1000);
    }
...
void TankGame::clear_picture() {
    ...
    for (int i = 0; i != depth; i++) {
        ...
        if (rx < wide - 1) {
            for (int k = u; k <= b; k++)
                printWin(rx, k,WHIHTE, "  ");
        }

        if (lx > 0) {
            for (int k = u; k <= b; k++)
                printWin(lx, k, WHIHTE,"  ");
        }

        if (by < high - 1) {
            for (int k = l; k <= r; k++)
                printWin(k, by,WHIHTE, "  ");
        }

        if (uy > 0) {
            for (int k = l; k <= r; k++)
                printWin(k, uy,WHIHTE, "  ");
        }
    }
}
    ...
    for (int y = 0; y != 40; y++) {
        for (int x = 0; x != 40; x++) {
            if (gameSpace[y][x].have)// && gameSpace[y][x].kind == 3)
                ...
                printWin(x, 42 + y,BLACK,(char) gameSpace[y][x].tankId);
            else
                printWin(x, 42 + y,BLACK,"  ");
        }
    }
}